...
def BEA(grafo:nx.Graph, raiz:str, llave:str, elemento):
    ...
    try:
        ...
        while len(por_procesar) > 0:
            ...
            #Como la lista por_procesar ya no contiene nodos, finaliza el algoritmo


        #Tiempo 1
        t1 = time.perf_counter()

        if llave_encontrada == True:
            ...
            return f"La llave '{llave}' no fue encontrada en el grafo. Tiempo: {(t1-t0)*1000}ms"
    except KeyError:
        return f"El nodo raiz {raiz} no se encuentra en el grafo. Tiempo: {(time.perf_counter()-t0)*1000}ms"